...
install_checkuser() {
    local latest_release=$(curl -s https://api.github.com/repos/Cyber-ssh/CheckerDtunnel-GO/releases/latest | grep "tag_name" | cut -d'"' -f4)
    local arch=$(get_arch)
    ...
    echo "Baixando $name..."
    wget -q "https://Github.com/Cyber-ssh/CheckerDtunnel-GO/releases/download/$latest_release/$name" -O /usr/local/bin/checkuser
